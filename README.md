...
Installing _qgis_ is not difficult either. For instance, on
//Ubuntu//, all it takes is `sudo apt-get install qgis`.  Also see the
[QGIS english download
page](https://www.qgis.org/en/site/forusers/download.html).  Make sure
you also have the extension //Raster -> Geo referencing// available.
...
**Without preparing at least //kethira.tif//, you will only see an
  empty page!**
...
create data images, the //png// format is preferred, because it allows
...
file by pressing the //Go// icon (second menu icon from the left).
This will be a so-called //GeoTIFF// image.  First, select the
//destination raster//.  This should be the corresponding entry in
`GEOSERVER_HOME/data_dir/images/`, so _geoserver_ can immediately pick
it up. Second, select //EPSG:4326// as //Destination CRS//. This
